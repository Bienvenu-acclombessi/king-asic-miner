/**
 * ProductWizardEdit - Wizard class for editing products
 * Extends functionality to work with existing product data
 */
class ProductWizardEdit {
    constructor() {
        // Load existing product data
        const productData = window.wizardData.productData;
        ...
        this.formData = {
            ...
            product_options: productData.productOptions || [],
            variants: productData.variants || [],
            collections: productData.collections?.map(c => c.id) || [],
            tags: productData.tags?.map(t => t.id) || [],
            minable_coins: productData.minableCoins || [],
            customer_groups: productData.customerGroups?.map(g => g.id) || [],
            meta_title: productData.meta_title || '',
            meta_description: productData.meta_description || '',
            meta_keywords: productData.meta_keywords || '',
            thumbnail: null, // Will be set if uploading new
            gallery_images: [], // Will be set if uploading new
            deleted_media_ids: [] // Track deleted images
        };
     ...
     */
    populateFormFields() {
        // Step 1 fields are already populated via blade value attributes
        // Just need to populate step 2 and beyond when user reaches those steps
    }

    /**
     * Load existing variants from product data
     */
    loadExistingVariants() {
        // Load from window.wizardData.productData which is already available
        const productData = window.wizardData.productData;

        if (productData && productData.variants && productData.variants.length > 0) {
            this.formData.variants = productData.variants.map(v => ({
                id: v.id,
                sku: v.sku || '',
                stock: v.stock || 0,
            ...
            console.log('Loaded variants from productData:', this.formData.variants);
        } else {
            // Try fallback to hidden field
            const existingVariantsData = document.getElementById('existingVariantsData');
            if (existingVariantsData && existingVariantsData.value) {
                try {
                    const variants = JSON.parse(existingVariantsData.value);
                    if (variants && variants.length > 0) {
                        this.formData.variants = variants.map(v => ({
                            id: v.id,
                            sku: v.sku || '',
                            stock: v.stock || 0,
                            enabled: v.enabled !== false,
        ...
        // If no variants exist, create default one
    ...
    setupStep5Listeners() {
        ...
        document.querySelectorAll('.remove-gallery-image').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                this.removeExistingGalleryImage(mediaId);
            });
        });
    }
    ...
    collectCurrentStepData() {
        ...
        if (step === 1) {
            ...
            this.formData.is_featured = document.getElementById('is_featured')?.checked || false;
            ...
            console.log('DEBUG: Found attribute inputs:', attributeInputs.length);
            ...
            attributeInputs.forEach(input => {
                ...
                console.log(`DEBUG: Attribute ${attributeHandle} = ${value}`);
            ...
            console.log('DEBUG: Collected attributes:', this.formData.attributes);
    ...
    prepareStep(step) {
        if (step === 2) {
            // Render existing options when entering step 2
            this.renderOptions();
        } else if (step === 3) {
            this.renderVariants();
        } else if (step === 4) {
    ...
    renderOptions() {
        ...
        const existingOptionsAlert = document.getElementById('existingOptionsAlert');

        if (this.formData.product_options.length === 0) {
            container.innerHTML = '';
            if (noOptionsAlert) noOptionsAlert.style.display = 'block';
            if (existingOptionsAlert) existingOptionsAlert.style.display = 'none';
            return;
        }

        if (noOptionsAlert) noOptionsAlert.style.display = 'none';
        if (existingOptionsAlert) existingOptionsAlert.style.display = 'none';
    ...
    // Copy remaining methods from product-wizard.js
    generateVariants() {
    ...
    renderThumbnail() {
        ...
        if (!this.formData.thumbnail) {
            // No change needed if showing existing thumbnail from blade
            return;
        }
        ...
        preview.innerHTML = `
            <div class="position-relative d-inline-block">
                <img src="${url}" alt="Thumbnail" class="img-fluid rounded mb-2" style="max-height: 300px; object-fit: cover;">
                <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0 m-2" onclick="wizard.removeThumbnail()">
                    <i class="bi bi-trash"></i>
                </button>
            </div>
            <p class="text-muted mt-2 mb-2">New thumbnail (will replace existing)</p>
    ...
    renderGallery() {
        ...
        let html = gallery.innerHTML; // Keep existing images
            ...
            html += `
                <div class="col-md-3">
                    <div class="position-relative">
                        <img src="${url}" alt="${file.name}" class="img-fluid rounded" style="width: 100%; height: 150px; object-fit: cover;">
                        <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0 m-2" onclick="wizard.removeGalleryImage(${index})">
                            <i class="bi bi-trash"></i>
                        </button>
                    </div>
    ...
    removeThumbnail() {
        ...
        // Reload the page section or reset to show existing
        location.reload();
    }
    ...
    removeExistingThumbnail() {
        if (confirm('Remove current thumbnail?')) {
            // Add logic to mark for deletion
            const preview = document.getElementById('thumbnailPreview');
            preview.innerHTML = `
                <i class="bi bi-image" style="font-size: 3rem; color: #6c757d;"></i>
                <p class="text-muted mt-2 mb-2">Thumbnail will be removed</p>
                <button type="button" class="btn btn-primary btn-sm" id="browseThumbnailBtn">
                    <i class="bi bi-folder2-open"></i> Select New Image
                </button>
            `;
            ...
            this.formData.remove_thumbnail = true;
        }
    }

    removeExistingGalleryImage(mediaId) {
        if (confirm('Remove this image?')) {
            this.formData.deleted_media_ids.push(mediaId);
    ...
    renderSummary() {
        ...
        const collectionNames = collections.map(collId => {
            ...
            if (collection) {
                // Collections use attribute_data.name
                const value = collection.attribute_data?.name || collection.name;
                return (typeof value === 'object' && value !== null)
                    ? (value.en || value.fr || Object.values(value)[0] || `Collection #${collId}`)
        ...
        let html = `
            <div class="row">
                <div class="col-md-6">
                    <p><strong>Product Name:</strong> ${this.formData.name || 'N/A'}</p>
                    <p><strong>Status:</strong> <span class="badge bg-${this.formData.status === 'published' ? 'success' : 'secondary'}">${this.formData.status}</span></p>
                    <p><strong>Type:</strong> ${productTypeName}</p>
                    <p><strong>Brand:</strong> ${brandName || 'N/A'}</p>
                </div>
                <div class="col-md-6">
                    <p><strong>Featured:</strong> ${this.formData.is_featured ? 'Yes' : 'No'}</p>
                    <p><strong>Options:</strong> ${(this.formData.product_options || []).length} option(s)</p>
                    <p><strong>Variants:</strong> ${(this.formData.variants || []).length} variant(s)</p>
                    <p><strong>Collections:</strong> ${collections.length > 0 ? collectionNames.join(', ') : 'None'}</p>
                </div>
            </div>
        `;
    ...
    async submitForm() {
        ...
        if (this.formData.remove_thumbnail) {
        ...
        // Deleted media
        this.formData.deleted_media_ids.forEach(id => {
        ...
        try {
            const response = await fetch(window.wizardData.updateUrl, {
                method: 'POST',
                body: formData,
                headers: {
