<div class="step-content">
    ...
    <p class="text-muted mb-4">Update the basic details about your product</p>

    <div class="row">
        <!-- Product Name -->
        <div class="col-md-6 mb-3">
            <label class="form-label">Product Name <span class="text-danger">*</span></label>
            <input type="text" class="form-control" name="name" id="name" value="{{ old('name', $productData['name'] ?? '') }}" required>
        ...
        <div class="col-md-6 mb-3">
            ...
            <input type="text" class="form-control" name="slug" id="slug" value="{{ old('slug', $productData['slug'] ?? '') }}" required>
        ...
        <div class="col-12 mb-3">
            ...
            <textarea class="form-control" name="short_description" id="short_description" rows="2">{{ old('short_description', $productData['short_description'] ?? '') }}</textarea>
        </div>

        <!-- Description -->
        <div class="col-12 mb-3">
            <label class="form-label">Description</label>
            <textarea class="form-control" name="description" id="description" rows="4">{{ old('description', $productData['description'] ?? '') }}</textarea>
        </div>
    </div>
            ...
            @if(isset($attributes) && $attributes->count() > 0)
                @foreach($attributes as $attribute)
                    @php
                        ...
                        // Get existing value from product - check pivot table
                        $productAttribute = $product->attributes->firstWhere('id', $attribute->id);
                        $existingValue = $productAttribute?->pivot->value ?? '';
                    @endphp
                    ...
                    <div class="mb-3" data-attribute-id="{{ $attribute->id }}">
                        ...
                        @if($attrType === 'text' || $attrType === 'string')
                            <input type="text"
                                   ...
                                   value="{{ old('attributes.'.$attribute->handle, $existingValue) }}"
                        ...
                        @elseif($attrType === 'textarea')
                            <textarea class="form-control attribute-input"
                                      ...
                                      rows="2"
                                      placeholder="Enter {{ strtolower($attrName) }}"
                                      {{ $isRequired ? 'required' : '' }}>{{ old('attributes.'.$attribute->handle, $existingValue) }}</textarea>
                            ...
                            <select class="form-select attribute-input"
                                ...
                                @if(isset($config['options']) && is_array($config['options']))
                                    @foreach($config['options'] as $option)
                                        ...
                                        @endphp
                                        <option value="{{ $optionValue }}" {{ old('attributes.'.$attribute->handle, $existingValue) == $optionValue ? 'selected' : '' }}>
                                            {{ $optionLabel }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        @elseif($attrType === 'boolean' || $attrType === 'checkbox')
                            <div class="form-check form-switch">
                                <input class="form-check-input attribute-input"
                                       ...
                                       data-attribute-handle="{{ $attribute->handle }}"
                                       value="1"
                                       {{ old('attributes.'.$attribute->handle, $existingValue) == '1' ? 'checked' : '' }}>
                            ...
                            <div class="card-body p-3">
                                <div class="form-check">
                                    <input class="form-check-input minable-coin-checkbox"
                                           ...
                                           {{ in_array($coin->id, $productData['minableCoins'] ?? []) ? 'checked' : '' }}>
